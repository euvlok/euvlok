...
fn ensureRaycastDefaults(rt: *script.Runtime) !void {
    ...
    try cf.addStringToAppArrayPreference(domain, "onboarding_completedTaskIdentifiers", "windowManagement");
    try cf.addStringToAppArrayPreference(domain, "commandsPreferencesExpandedItemIds", "builtin_package_windowManagement");
}
...
fn raycastPids(rt: *script.Runtime) ![]std.posix.pid_t {
    return try macos.pidsForExecutablePath(rt.io, rt.allocator, raycast_bin);
}
